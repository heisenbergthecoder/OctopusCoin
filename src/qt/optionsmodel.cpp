...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeOctopusAmount"))
        settings.setValue("nAnonymizeOctopusAmount", 1000);
    nAnonymizeOctopusAmount = settings.value("nAnonymizeOctopusAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeOctopusAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeOctopusAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeOctopusAmount:
            return QVariant(nAnonymizeOctopusAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeOctopusAmount:
            nAnonymizeOctopusAmount = value.toInt();
            settings.setValue("nAnonymizeOctopusAmount", nAnonymizeOctopusAmount);
            emit AnonymizeOctopusAmountChanged(nAnonymizeOctopusAmount);
            break;
        default:
            break;
        }
