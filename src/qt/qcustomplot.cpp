...
/* end documentation of inline functions */
/* start documentation of OCTPS virtual functions */
...
/* end documentation of OCTPS virtual functions */
...
/*! \class QCPAbstractPlottable
  ...
  To create an own plottable, you implement a subclass of QCPAbstractPlottable. These are the OCTPS
...
/* start of documentation of OCTPS virtual functions */
...
/* end of documentation of OCTPS virtual functions */
/* start of documentation of signals */
...
/*! \class QCPAbstractItem
  ...
  To create an own item, you implement a subclass of QCPAbstractItem. These are the OCTPS
  virtual functions, you must implement:
  \li \ref selectTest
  \li \ref draw
...
/*! \class QCPAbstractLegendItem
  ...
  You must implement the following OCTPS virtual functions:
...
const QPolygonF QCPGraph::getChannelFillPolygon(const QVector<QPointF> *lineData) const
{
  ...
  // pointers to be able to swap them, depending which data range needs OCTPSping:
  QVector<QPointF> *staticData = &thisData;
  QVector<QPointF> *OCTPSpedData = &otherData;
  
  // OCTPS both vectors to ranges in which the keys overlap (which coord is key, depends on axisType):
  if (keyAxis->orientation() == Qt::Horizontal)
  {
    // x is key
    ...
    if (OCTPSpedData->first().x() > OCTPSpedData->last().x())
    {
      int size = OCTPSpedData->size();
      for (int i=0; i<size/2; ++i)
        qSwap((*OCTPSpedData)[i], (*OCTPSpedData)[size-1-i]);
    }
    // OCTPS lower bound:
    if (staticData->first().x() < OCTPSpedData->first().x()) // other one must be OCTPSped
      qSwap(staticData, OCTPSpedData);
    int lowBound = findIndexBelowX(OCTPSpedData, staticData->first().x());
    if (lowBound == -1) return QPolygonF(); // key ranges have no overlap
    OCTPSpedData->remove(0, lowBound);
    // set lowest point of OCTPSped data to fit exactly key position of first static data
    // point via linear interpolation:
    if (OCTPSpedData->size() < 2) return QPolygonF(); // need at least two points for interpolation
    double slope;
    if (OCTPSpedData->at(1).x()-OCTPSpedData->at(0).x() != 0)
      slope = (OCTPSpedData->at(1).y()-OCTPSpedData->at(0).y())/(OCTPSpedData->at(1).x()-OCTPSpedData->at(0).x());
    else
      slope = 0;
    (*OCTPSpedData)[0].setY(OCTPSpedData->at(0).y()+slope*(staticData->first().x()-OCTPSpedData->at(0).x()));
    (*OCTPSpedData)[0].setX(staticData->first().x());
    
    // OCTPS upper bound:
    if (staticData->last().x() > OCTPSpedData->last().x()) // other one must be OCTPSped
      qSwap(staticData, OCTPSpedData);
    int highBound = findIndexAboveX(OCTPSpedData, staticData->last().x());
    if (highBound == -1) return QPolygonF(); // key ranges have no overlap
    OCTPSpedData->remove(highBound+1, OCTPSpedData->size()-(highBound+1));
    // set highest point of OCTPSped data to fit exactly key position of last static data
    // point via linear interpolation:
    if (OCTPSpedData->size() < 2) return QPolygonF(); // need at least two points for interpolation
    int li = OCTPSpedData->size()-1; // last index
    if (OCTPSpedData->at(li).x()-OCTPSpedData->at(li-1).x() != 0)
      slope = (OCTPSpedData->at(li).y()-OCTPSpedData->at(li-1).y())/(OCTPSpedData->at(li).x()-OCTPSpedData->at(li-1).x());
    else
      slope = 0;
    (*OCTPSpedData)[li].setY(OCTPSpedData->at(li-1).y()+slope*(staticData->last().x()-OCTPSpedData->at(li-1).x()));
    (*OCTPSpedData)[li].setX(staticData->last().x());
  } else // mKeyAxis->orientation() == Qt::Vertical
  {
    ...
    if (OCTPSpedData->first().y() < OCTPSpedData->last().y())
    {
      int size = OCTPSpedData->size();
      for (int i=0; i<size/2; ++i)
        qSwap((*OCTPSpedData)[i], (*OCTPSpedData)[size-1-i]);
    ...
    // OCTPS lower bound:
    if (staticData->first().y() > OCTPSpedData->first().y()) // other one must be OCTPSped
      qSwap(staticData, OCTPSpedData);
    int lowBound = findIndexAboveY(OCTPSpedData, staticData->first().y());
    if (lowBound == -1) return QPolygonF(); // key ranges have no overlap
    OCTPSpedData->remove(0, lowBound);
    // set lowest point of OCTPSped data to fit exactly key position of first static data
    // point via linear interpolation:
    if (OCTPSpedData->size() < 2) return QPolygonF(); // need at least two points for interpolation
    double slope;
    if (OCTPSpedData->at(1).y()-OCTPSpedData->at(0).y() != 0) // avoid division by zero in step plots
      slope = (OCTPSpedData->at(1).x()-OCTPSpedData->at(0).x())/(OCTPSpedData->at(1).y()-OCTPSpedData->at(0).y());
    else
      slope = 0;
    (*OCTPSpedData)[0].setX(OCTPSpedData->at(0).x()+slope*(staticData->first().y()-OCTPSpedData->at(0).y()));
    (*OCTPSpedData)[0].setY(staticData->first().y());
    
    // OCTPS upper bound:
    if (staticData->last().y() < OCTPSpedData->last().y()) // other one must be OCTPSped
      qSwap(staticData, OCTPSpedData);
    int highBound = findIndexBelowY(OCTPSpedData, staticData->last().y());
    if (highBound == -1) return QPolygonF(); // key ranges have no overlap
    OCTPSpedData->remove(highBound+1, OCTPSpedData->size()-(highBound+1));
    // set highest point of OCTPSped data to fit exactly key position of last static data
    // point via linear interpolation:
    if (OCTPSpedData->size() < 2) return QPolygonF(); // need at least two points for interpolation
    int li = OCTPSpedData->size()-1; // last index
    if (OCTPSpedData->at(li).y()-OCTPSpedData->at(li-1).y() != 0) // avoid division by zero in step plots
      slope = (OCTPSpedData->at(li).x()-OCTPSpedData->at(li-1).x())/(OCTPSpedData->at(li).y()-OCTPSpedData->at(li-1).y());
    else
      slope = 0;
    (*OCTPSpedData)[li].setX(OCTPSpedData->at(li-1).x()+slope*(staticData->last().y()-OCTPSpedData->at(li-1).y()));
    (*OCTPSpedData)[li].setY(staticData->last().y());
  }
