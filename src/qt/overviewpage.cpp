...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeOctopusAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeOctopusAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeOctopusAmount = strAnonymizeOctopusAmount.remove(strAnonymizeOctopusAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeOctopusAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeOctopusAmount*COIN) nMaxToAnonymize = nAnonymizeOctopusAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeOctopusAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeOctopusAmount));
        strAnonymizeOctopusAmount = strAnonymizeOctopusAmount.remove(strAnonymizeOctopusAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeOctopusAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
