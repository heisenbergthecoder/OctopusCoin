...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: OCTPS.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: OCTPSd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeOCTPSamount=<n> " + _("Keep N Octopus anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Octopus is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Octopus is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Octopus version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Octopus server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Octopus") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Octopus to complete") << "\n";
    ...
    nAnonymizeOctopusAmount = GetArg("-anonymizeOCTPSamount", 0);
    if(nAnonymizeOctopusAmount > 999999) nAnonymizeOctopusAmount = 999999;
    if(nAnonymizeOctopusAmount < 2) nAnonymizeOctopusAmount = 2;
    ...
    LogPrintf("Anonymize Octopus Amount %d\n", nAnonymizeOctopusAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1OCTPS+1000 == (.1OCTPS+100)*10
       10OCTPS+10000 == (1OCTPS+1000)*10
    */
