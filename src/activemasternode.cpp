...
//
// Bootup the masternode, look for a 1000 OCTPS input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in OCTPS.conf */
...
{
    COctopuscoinAddress address(collateralAddress);
